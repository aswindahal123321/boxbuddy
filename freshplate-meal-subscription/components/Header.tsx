  ...
  return (
    <header className="bg-white/80 backdrop-blur-sm p-4 border-b border-slate-200 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <button onClick={() => setCurrentPage('home')} className="text-2xl font-bold text-emerald-600">
          FreshPlate
        </button>
        <nav className="hidden md:flex items-center space-x-6">
          <button onClick={() => setCurrentPage('home')} className="text-slate-600 hover:text-emerald-600 transition-colors">Home</button>
          <a href="#menu" onClick={(e) => { e.preventDefault(); setCurrentPage('home'); setTimeout(() => document.getElementById('menu')?.scrollIntoView({ behavior: 'smooth' }), 0); }} className="text-slate-600 hover:text-emerald-600 transition-colors">Meals</a>
